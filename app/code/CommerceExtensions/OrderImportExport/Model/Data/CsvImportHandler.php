...
class CsvImportHandler
{
    ...
    public function processOrder(&$orderData, $importData, $params)
    {
        ...
        try {
			...
			*/
            ...
            $this->updateShippingDescription($importData, $order1, $connection, $resource);
     ...
     */
    public function updateShippingDescription($importData, $order1, $connection, $resource)
    {
        if(isset($importData['shipping_description'])) {
            if($importData['shipping_description'] != '') {
                $shipping = str_replace('"','',$importData['shipping_description']);
                $salesOrderTable = $resource->getTableName('sales_order');
                $sql = "UPDATE `{$salesOrderTable}` SET shipping_description = '{$shipping}' WHERE entity_id = {$order1->getId()}";
